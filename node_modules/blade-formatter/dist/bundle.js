...
`),o=a.amount/(this.indentCharacter==="	"?4:1),c=l[0],d=this.indentCharacter.repeat(o<0?0:o),p=l.map(m=>d+m);return p[0]=c,`${p.join(`
`)}`})}restoreTemplatingString(e){return r.replace(e,new RegExp(`${this.getTemplatingStringPlaceholder("(\\d+)")}`,"gms"),(t,n)=>this.templatingStrings[n])}restoreStringLiteralInPhp(e){return r.replace(e,new RegExp(`${this.getStringLiteralInPhpPlaceholder("(\\d+)")}`,"gms"),(t,n)=>this.stringLiteralInPhp[n])}async restoreComponentAttribute(e){return x(e,new RegExp(`${this.getComponentAttributePlaceholder("(\\d+)")}`,"gim"),async(t,n)=>{var c;let i=this.getComponentAttributePlaceholder(n),s=(c=e.match(new RegExp(`^(.*?)${i}`,"gmi")))!=null?c:[""],a=_(s[0]),l=this.componentAttributes[n],o=await x(l,/(:{1,2}.*?=)(["'])(.*?)(?=\2)/gis,async(d,p,u,m)=>{if(m==="")return d;if(s[0].startsWith("<livewire"))return`${p}${u}${m}`;if(p.startsWith("::"))return`${p}${u}${D.js_beautify(m,{wrap_line_length:this.wrapLineLength-a.amount,brace_style:"preserve-inline"}).trim()}`;if(this.isInline(m))try{return`${p}${u}${(await C(m,{...this.options,printWidth:this.wrapLineLength-a.amount})).trimEnd()}`}catch{return`${p}${u}${m}`}return`${p}${u}${(await C(m,{...this.options,printWidth:this.wrapLineLength-a.amount})).trimEnd()}`});return`${this.indentComponentAttribute(a.indent,o)}`})}restoreShorthandBinding(e){return r.replace(e,new RegExp(`${this.getShorthandBindingPlaceholder("(\\d+)")}`,"gms"),(t,n)=>{var c;let i=this.getShorthandBindingPlaceholder(n),s=(c=e.match(new RegExp(`^(.*?)${i}`,"gmi")))!=null?c:[""],a=_(s[0]),l=this.shorthandBindings[n],o=r.replace(l,/(:{1,2}.*?=)(["'])(.*?)(?=\2)/gis,(d,p,u,m)=>{let $={wrap_line_length:this.wrapLineLength-a.amount,brace_style:"preserve-inline"};if(m==="")return d;if(this.isInline(m))try{return`${p}${u}${D.js_beautify(m.trim(),$).trim()}`}catch{return`${p}${u}${m.trim()}`}return`${p}${u}${D.js_beautify(m.trim(),$).trim()}`});return`${this.indentComponentAttribute(a.indent,o)}`})}async formatAsBlade(e){this.currentIndentLevel=0,this.shouldBeIndent=!1;let t=W(e),n=await new K(this.vsctm,this.oniguruma);return n.createRegistry().loadGrammar("text.html.php.blade").then(a=>n.tokenizeLines(t,a)).then(a=>this.formatTokenizedLines(t,a)).catch(a=>{throw a})}async formatTokenizedLines(e,t){this.result=[],this.stack=[];for(let n=0;n<e.length;n+=1){let i=e[n],s=t[n];await this.processLine(s,i)}return this.result.join(this.endOfLine)}async processLine(e,t){await this.processTokenizeResult(e,t)}async processKeyword(e){if(r.includes(F,e)){r.last(this.stack)==="@case"&&e==="@case"&&this.decrementIndentLevel(),e==="@case"&&(this.shouldBeIndent=!0),this.stack.push(e);return}if(r.includes(X,e)){if(e==="@break"){this.decrementIndentLevel(),this.stack.pop(),this.stack.push(e);return}if(r.last(this.stack)!=="@hassection"){this.stack.pop();return}}if(r.includes(oe,e)&&(this.shouldBeIndent=!0,this.stack.push(e)),r.includes(Z,e)&&r.includes(ae,r.last(this.stack))&&(this.decrementIndentLevel(),this.shouldBeIndent=!0),r.includes(P,e)&&(r.last(this.stack)==="@section"&&e==="@section"?(this.currentIndentLevel>0&&this.decrementIndentLevel(),this.shouldBeIndent=!0,this.stack.push(e)):(this.shouldBeIndent=!0,this.stack.push(e))),r.includes(R,e)){if(e==="@endswitch"&&r.last(this.stack)==="@default"){this.decrementIndentLevel(2),this.shouldBeIndent=!1;return}this.decrementIndentLevel(),this.shouldBeIndent=!1,this.stack.pop()}r.includes(E,e)&&(this.decrementIndentLevel(),this.shouldBeIndent=!0)}async processToken(e,t){if(r.includes(e.scopes,"punctuation.definition.comment.begin.blade")&&(this.isInsideCommentBlock=!0),this.argumentCheck){let{count:n,inString:i,stack:s,unindentOn:a}=this.argumentCheck;if(!i&&t===")"){if(s.push(t),n[t]+=1,n["("]===n[t]){let l=s.join("");await de(l)>=a&&(this.shouldBeIndent=!1),this.argumentCheck=!1}return}s.push(t),i===t?this.argumentCheck.inString=!1:!i&&(t==='"'||t==="'")&&(this.argumentCheck.inString=t),t==="("&&!i&&(n[t]+=1)}r.includes(e.scopes,"punctuation.definition.comment.end.blade")&&(this.isInsideCommentBlock=!1),(t==="{{--"||t.includes("{{--"))&&(this.isInsideCommentBlock=!0),(t==="--}}"||t.includes("--}}"))&&(this.isInsideCommentBlock=!1),r.includes(e.scopes,"keyword.blade")&&(this.isInsideCommentBlock||(await this.processKeyword(t.toLowerCase()),r.includes(Object.keys(G),t.toLowerCase())&&(this.argumentCheck={unindentOn:G[t.toLowerCase()],stack:[],inString:!1,count:{"(":0,")":0}})))}async processTokenizeResult(e,t){if(this.shouldBeIndent&&(this.incrementIndentLevel(),this.shouldBeIndent=!1),ce(e,t)){this.insertFormattedLineToResult(t);return}for(let n=0;n<e.tokens.length;n+=1){let i=e.tokens[n],s=t.substring(i.startIndex,i.endIndex).trim();await this.processToken(i,s)}this.insertFormattedLineToResult(t)}insertFormattedLineToResult(e){let n=_(e).amount+this.indentSize*this.currentIndentLevel,i=this.indentCharacter.repeat(n<0?0:n)+e.trim();e.length===0&&this.result.push(e),e.length!==0&&i.length>0&&this.result.push(i),i!==e&&this.diffs.push({original:e,formatted:i})}incrementIndentLevel(e=1){this.currentIndentLevel+=e}decrementIndentLevel(e=1){this.currentIndentLevel-=e}async formatExpressionInsideBladeDirective(e,t,n=void 0){let i=`func(${e})`,s=await C(i,{...this.options,printWidth:n!=null?n:this.defaultPhpFormatOption.printWidth});if(s===i)return e;let a=s.replace(/([\n\s]*)->([\n\s]*)/gs,"->").replace(/(?<!(['"]).*)(?<=\()[\n\s]+?(?=\w)/gm,"").replace(/([^]*)],[\n\s]*?\)$/gm,(l,o)=>`${o}]
)`).replace(/,[\n\s]*?\)/gs,")").replace(/,(\s*?\))$/gm,(l,o)=>o).trim();return this.options.useTabs&&(a=r.replace(a,/(?<=^ *) {4}/gm,"	".repeat(this.indentSize))),a=a.replace(/func\((.*)\)/gis,(l,o)=>o),this.isInline(a.trim())&&(a=a.trim()),this.indentRawPhpBlock(t,a)}formatJS(e){let t=e,n={js:[],entangle:[]};return Object.keys(n).forEach(i=>{t=t.replace(new RegExp(`@${i}\\((?:[^)(]+|\\((?:[^)(]+|\\([^)(]*\\))*\\))*\\)`,"gs"),s=>{let a=n[i].push(s)-1;return this.getPlaceholder(i,a,s.length)})}),t=D.js_beautify(t,{brace_style:"preserve-inline"}),Object.keys(n).forEach(i=>{t=t.replace(new RegExp(this.getPlaceholder(i,"_*(\\d+)"),"gms"),(s,a)=>n[i][a])}),t}};import Ve from"ajv";import Ke from"find-config";import Qe from"fs";import Ge from"path";var Ze=new Ve,$e=[".bladeformatterrc.json",".bladeformatterrc"];function be(h){for(let e=0;e<$e.length;e+=1){let t=Ke($e[e],{cwd:Ge.dirname(h),home:!1});if(t)return t}return null}async function xe(h){if(h===null)return;let e=JSON.parse((await Qe.promises.readFile(h)).toString()),t={type:"object",properties:{indentSize:{type:"integer",nullable:!0},wrapLineLength:{type:"integer",nullable:!0},wrapAttributes:{type:"string",enum:["auto","force","force-aligned","force-expand-multiline","aligned-multiple","preserve","preserve-aligned"],nullable:!0},wrapAttributesMinAttrs:{type:"integer",nullable:!0,default:2},indentInnerHtml:{type:"boolean",nullable:!0},endWithNewline:{type:"boolean",nullable:!0},endOfLine:{type:"string",enum:["LF","CRLF"],nullable:!0},useTabs:{type:"boolean",nullable:!0},sortTailwindcssClasses:{type:"boolean",nullable:!0},tailwindcssConfigPath:{type:"string",nullable:!0},sortHtmlAttributes:{type:"string",enum:["none","alphabetical","code-guide","idiomatic","vuejs","custom"],nullable:!0},customHtmlAttributesOrder:{type:"array",nullable:!0,items:{type:"string"},default:[]},noMultipleEmptyLines:{type:"boolean",nullable:!0},noPhpSyntaxCheck:{type:"boolean",nullable:!0},noSingleQuote:{type:"boolean",nullable:!0},noTrailingCommaPhp:{type:"boolean",nullable:!0},extraLiners:{type:"array",nullable:!0,items:{type:"string"},default:["head","body","/html"]}},additionalProperties:!0},n=Ze.compile(t);if(!n(e))throw n;return e}var Be=class h{constructor(e={},t=[]){var n;this.currentTargetPath=".",this.paths=t,this.options=e,this.targetFiles=[],this.errors=[],this.diffs=[],this.outputs=[],this.formattedFiles=[],this.ignoreFile="",this.fulFillFiles=[],this.targetFiles=[],this.runtimeConfigPath=(n=e.runtimeConfigPath)!=null?n:null,this.runtimeConfigCache={}}async format(e,t={}){this.options=this.options||t;let n=I.resolve(g.cwd(),"target");return await this.readIgnoreFile(n),await this.findTailwindConfig(n),await this.readRuntimeConfig(n),new O(this.options).formatContent(e).catch(i=>{throw new j(i)})}async formatFromCLI(){try{this.printPreamble(),await this.readIgnoreFile(g.cwd()),await this.processPaths(),this.printResults()}catch{}}fileExists(e){return z.promises.access(e,z.constants.F_OK).then(()=>!0).catch(()=>!1)}async readIgnoreFile(e){let t=".bladeignore",n,i=I.dirname(e);if(this.options.ignoreFilePath?n=this.options.ignoreFilePath:n=Se(t,{cwd:i}),!!n)try{this.ignoreFile=(await z.promises.readFile(n)).toString()}catch{}}async findTailwindConfig(e){if(!this.options.sortTailwindcssClasses)return;let t="tailwind.config.js",n;if(this.options.tailwindcssConfigPath)if(this.runtimeConfigPath){let i=I.dirname(this.runtimeConfigPath);n=I.resolve(i,this.options.tailwindcssConfigPath)}else I.isAbsolute(this.options.tailwindcssConfigPath)?n=I.resolve(this.options.tailwindcssConfigPath):n=I.resolve(this.options.tailwindcssConfigPath);else{let i=I.dirname(e);n=Se(t,{cwd:i})}n&&(this.options.tailwindcssConfigPath=n)}async readRuntimeConfig(e){var n,i;w.isEmpty(this.runtimeConfigCache)&&(this.options=w.merge(this.options,this.runtimeConfigCache));let t;if(this.options.runtimeConfigPath?t=this.options.runtimeConfigPath:t=be(e),!w.isNull(t)){this.runtimeConfigPath=t;try{let s=await xe(t);this.options=w.mergeWith(this.options,s,(a,l)=>w.isNil(l)?a:l),this.runtimeConfigCache=this.options,this.options.sortTailwindcssClasses&&await this.findTailwindConfig(e)}catch(s){s instanceof SyntaxError&&(g.stdout.write(b.red.bold(`
Blade Formatter JSON Syntax Error: 

`)),g.stdout.write(Ce.format(s)),g.exit(1)),g.stdout.write(b.red.bold(`
Blade Formatter Config Error: ${I.basename(t)}

`)),g.stdout.write(`\`${s.errors[0].instancePath.replace("/","")}\` ${s.errors[0].message}

`),(n=s.errors[0].params)!=null&&n.allowedValues&&console.log((i=s.errors[0].params)==null?void 0:i.allowedValues),g.exit(1)}}}async processPaths(){await Promise.all(w.map(this.paths,async e=>this.processPath(e)))}async processPath(e){await h.globFiles(e).then(t=>w.map(t,n=>I.relative(".",n))).then(t=>this.filterFiles(t)).then(this.fulFillFiles).then(t=>this.formatFiles(t))}static globFiles(e){return new Promise((t,n)=>{Ye(e,(i,s)=>i?n(i):t(s))})}async filterFiles(e){if(this.ignoreFile==="")return e;let t=/^\.\.*/,n=w.filter(e,a=>t.test(I.relative(".",a))),i=w.xor(e,n),s=qe().add(this.ignoreFile).filter(i);return w.concat(n,s)}static fulFillFiles(e){return this.targetFiles.push(e),Promise.resolve(e)}async formatFiles(e){await Promise.all(w.map(e,async t=>this.formatFile(t)))}async formatFile(e){await this.findTailwindConfig(e),await this.readRuntimeConfig(e),await U(e).then(t=>Promise.resolve(t.toString("utf-8"))).then(t=>new O(this.options).formatContent(t)).then(t=>this.checkFormatted(e,t)).then(t=>this.writeToFile(e,t)).catch(t=>{this.handleError(e,t)})}async checkFormatted(e,t){this.printFormattedOutput(e,t);let n=z.readFileSync(e,"utf-8"),i=W(n),s=W(t),a=me(e,i,s);return this.diffs.push(a),this.outputs.push(t),a.length>0&&((this.options.progress||this.options.write)&&g.stdout.write(b.green("F")),this.options.checkFormatted&&(g.stdout.write(`${e}
`),g.exitCode=1),this.formattedFiles.push(e)),a.length===0&&(this.options.progress||this.options.write)&&g.stdout.write(b.green(".")),Promise.resolve(t)}printFormattedOutput(e,t){this.options.write||this.options.checkFormatted||(g.stdout.write(`${t}`),w.last(this.paths)===e||w.last(this.targetFiles)===e)||(this.paths.length>1||this.targetFiles.length>1)&&g.stdout.write(`
`)}writeToFile(e,t){this.options.write&&(this.options.checkFormatted||t.length===0||w.isNull(t)||w.isEmpty(t)||z.writeFile(e,t,n=>{n&&(g.stdout.write(`${b.red(n.message)}
`),g.exit(1))}))}handleError(e,t){(this.options.progress||this.options.write)&&g.stdout.write(b.red("E")),g.exitCode=1,this.errors.push({path:e,message:t.message,error:t})}printPreamble(){this.options.checkFormatted&&g.stdout.write(`Check formatting... 
...
`;g.stdout.write(e),g.stdout.write(b.bold.green(`Fixed: F
`)),g.stdout.write(b.bold.red(`Errors: E
`)),g.stdout.write(b.bold("Not Changed: ")+b.bold.green(`.
`))}printFormattedFiles(){if(this.formattedFiles.length===0){this.options.checkFormatted&&g.stdout.write(b.bold(`
...
`);return}g.stdout.write(b.bold(`
Formatted Files: 
`)),w.each(this.formattedFiles,e=>g.stdout.write(`${b.bold(e)}
`))}printDifferences(){if(this.options.diff){if(g.stdout.write(b.bold(`
Differences: 

`)),w.filter(this.diffs,e=>e.length>0).length===0){g.stdout.write(b(`No changes found. 

`));return}w.each(this.diffs,e=>ue(e))}}printErrors(){w.isEmpty(this.errors)||(g.stdout.write(b.red.bold(`
Errors: 

`)),w.each(this.errors,e=>g.stdout.write(`${Ce.format(e)}
`)))}};export{Be as BladeFormatter,O as Formatter};
