...
var M=(h=>typeof require<"u"?require:typeof Proxy<"u"?new Proxy(h,{get:(e,t)=>(typeof require<"u"?require:e)[t]}):h)(function(h){if(typeof require<"u")return require.apply(this,arguments);throw Error('Dynamic require of "'+h+'" is not supported')});import X from"chalk";import st from"concat-stream";import oe from"lodash";import{loadWASM as at}from"vscode-oniguruma";import ot from"yargs";import{promises as lt}from"fs";import ct from"os";import{hideBin as ht}from"yargs/helpers";var Z="blade-formatter";var ce="1.40.1";import it from"ignore";import $ from"chalk";import De from"find-config";import z from"fs";import rt from"glob";import w from"lodash";import I from"path";import g from"process";import Le from"util";var O=class extends Error{};import{sortClasses as ae}from"@shufo/tailwindcss-class-sorter";import qe from"aigle";import _ from"detect-indent";import{sortAttributes as Be}from"html-attribute-sorter";import D from"js-beautify";import r from"lodash";import*as Ke from"vscode-textmate";import Ge from"xregexp";import x from"string-replace-async";function ke(h){return h.split(`
...
`)}writeToFile(e,t){this.options.write&&(this.options.checkFormatted||t.length===0||w.isNull(t)||w.isEmpty(t)||z.writeFile(e,t,n=>{n&&(g.stdout.write(`${$.red(n.message)}
...
`));return}w.each(this.diffs,e=>we(e))}}printErrors(){w.isEmpty(this.errors)||(g.stdout.write($.red.bold(`
Errors: 

`)),w.each(this.errors,e=>g.stdout.write(`${Le.format(e)}
`)))}};async function pt(){let h=await ot(ht(process.argv)).usage(`${X.green(Z)} ${ce}
An opinionated blade template formatter for Laravel. 

 ${X.yellow("Usage:")} $0 [options] [file glob | ...]`).wrap(null).example('$0 "resources/views/**/*.blade.php" --write',"Format all files in views directory").option("check-formatted",{alias:"c",type:"boolean",description:"Only checks files are formatted or not",default:!1}).option("write",{alias:"w",type:"boolean",description:"Write to file",default:!1}).option("diff",{alias:"d",type:"boolean",description:"Show diffs",default:!1}).option("end-with-newline",{alias:"e",type:"boolean",description:"End output with newline",default:!0}).option("end-of-line",{type:"string",description:"End of line character(s). [LF|CRLF]",default:ct.EOL===`\r
